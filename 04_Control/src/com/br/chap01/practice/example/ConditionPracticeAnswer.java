...
public class ConditionPracticeAnswer {
	...
	public void practice5() {
		...
		// 3. 사용자가 입력한 값들과 임의로 지정한값들이 일치하는지 조건검사
		String result = "둘 다 틀렸습니다.";
	...
	public void practice8() {
		...
		// 2. 연산하고자 하는 연산기호 입력받기
		System.out.print("연산자를 입력(+,-,*,/,%) : ");
		char op = sc.nextLine().charAt(0);
		
		// 3. 사용자가 입력한 두 정수값과 연산기호, 연산결과 출력
		if(num1 > 0 && num2 > 0) { // 두 수 모두 양수로 입력했을 경우
			
			// 연산기호가지고 그에 맞는 연산결과 출력
			int result = 0;
			switch(op) {
	...
	public void practice10() {
		...
		System.out.println("과제 점수(20) : " + homework);
		System.out.println("출석 점수(20) : " + attendance);
		System.out.printf("총점 : %.1f\n\n", total);
		
		if(total >= 80 && attendance >= 20 * 0.8) {
			// 80점이상이고 출석률이 80프로이상일 경우 => PASS
			System.out.println("PASS");
		}else {
			// 그게 아닐경우 => FAIL
			/*
			if(total >= 80) { // 총점은 통관데 출석률이 모자란 경우
				System.out.printf("FAIL [출석 횟수 부족] (%d/20)", (int)attendance);
			}else if(attendance >= 20 * 0.8) { // 출석률은 통과인데 총점이 미달일 경우
				System.out.printf("FAIL [점수 미달] (총점 %.1f)", total);
			}else { // 총점도 미달이고 출석률도 부족할 경우
			...
			*/
			...
			if(total < 80) { // 총점이 미달일 경우
				System.out.printf("FAIL [점수 미달] (총점 %.1f)\n", total);
			}
			
			if(attendance < 20 * 0.8) { // 출석률이 부족할 경우
				System.out.printf("FAIL [출석 횟수 부족] (%d/20)", (int)attendance);
			}
