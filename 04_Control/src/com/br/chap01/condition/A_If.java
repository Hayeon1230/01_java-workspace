...
public class A_If {
	...
	/*
	 * 기본적으로 위에서부터 아래로 순차적으로 진행되는 흐름을
	 * 직접 제어할 수 있음 => 제어문 활용
	 * 
	 * 선택적으로 실행하는 선택문 => 조건문
	 * 반복적으로 실행하는 실행문 => 반복문
	 ...
	 * 
	 * * 조건문
	 * 조건식(참/거짓 결과)을 통해 참이냐 거짓이냐를 판단해서
	 * 해당 조건에 만족할 경우 그에 해당하는 구문을 실행
	 * 
	 * * 조건문 종류
	 * 1. if문
	 * - 단독 if문
...
//		}else if(/*age > 13 &&*/ age <= 19) { // 이미 앞조건 false이므로 13초과임
...
//		}
	...
	public void method5() {
		...
		String result = "";//변수를 미리 세팅해둘 대는 반드시 초기화 해놓는 습관을 들이자!!
		if(gender == 'M' || gender == 'm') {
			result = "남학생";
		}else if(gender == 'f' || gender == 'F') {
			result = "여학생";
	...
	public void method8() {
		...
		// -포함하여 잘 입력했을 경우 => 성별 판별해서 여자/남자 출력
		//		잘못 입력했을 경우 => "-포함해서 입력해주세요" 출력
		
//		System.out.print("주민번호(-포함) : ");
//		char seven = sc.nextLine().charAt(7);
//		if(seven == '-') {
//			if(seven == '2' || seven == '4') {
