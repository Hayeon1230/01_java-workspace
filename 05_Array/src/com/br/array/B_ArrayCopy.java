...
public class B_ArrayCopy {
	...
	public void method1() {
		...
		// 얕은 복사 (주소값만을 복사 => 결국 같은곳을 참조)
		// 원본 수정하든 카피 수정하든 같은게 수정됨
	...
	public void method2() {
		...
		// 더 담으려 크기 넉넉하게
		for(int i=0; i<origin.length; i++) {//copy길이로 하면 오류
			copy[i] = origin[i];
		}// {1,2,3,4,5}
		copy[2] = 99;
		...
		System.out.println("\n\norigin의 주소값 : " + origin + " " + origin.hashCode());

		System.out.println("\n\ncopy의 주소값 : " + copy + " " + copy.hashCode());
		
	}
	
	public void method3() {
		// 2. 새로운 배열을 생성한 후 System클래스의 arraycopy메소드를 이용한 복사
		int[] origin = {1,2,3,4,5};
	...
	public void method4() {
		...
		int[] origin = {1,2,3,4,5};
		// 복사본 배열 = Arrays.copyOf(원본배열명, 복사할 길이(생성될 배열의 길이));//위에꺼처럼 인덱스 지정은 불가
		int[] copy = Arrays.copyOf(origin, 10); //new int x
		/*
	...
	public void method6() {
		...
		// 후 기존에 담겨있던 3개의 없이 복사
		arr = Arrays.copyOf(arr, arr.length + addSize);
		// 추가값갯수로 2 입력 가정하에 => {1,2,3,0,0}
		// arr.length ==> 기존배열의길이+추가값갯수
		
		for(int i=length/*기존의배열길이(복사전)(==마지막인덱스+1)*/; i<arr.length; i++) {
