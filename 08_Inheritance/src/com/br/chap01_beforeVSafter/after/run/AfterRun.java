...
public class AfterRun /*extends Object*/ {
	...
	public static void main(String[] args) {
		 ...
		 * - 부모클래스에 있는 메소드를 오버라이딩 해서 자식클래스에서 재정의 가능 
		 *   => 오버라이딩 하면 자식클래스에 있는 메소드가 우선권을 가짐
		 * - 클래스간의 상속은 다중 상속이 불가능하다 (단일상속만 가능)
		 * - 모든 클래스는 Object 클래스를 상속 받고 있다. 
		 *   즉, Object 클래스가 모든 클래스의 최상위 클래스 
		 *   => 즉, 모든 클래스는 Object클래스의 메소드들을 마치 내것처럼 호출 할 수 있음
		 *     => Object클래스에 있는 메소드가 맘에 안들면 재정의해서(오버라이딩) 쓸수 있음 
		 */
