...
public class B_KeyboardInput {
	...
	public void inputScanner2() {
		...
		System.out.print("이름 : ");
		String name = sc.nextLine();//엔터치면 입력받았구나 인식하고 버퍼->박스로 옮기고 엔터도 비워줌
		
		System.out.print("나이 : ");
		int age = sc.nextInt();// 사용자가 입력한 숫자값 뽑아지고 엔터는 비워주지 않음
		//엔터 안 비워줌 그래서 다음에 빈문자열 엔터로 들어감
		...
		// xxx님은 xx살이며, 사는 곳은 xx이고 키는 xxx.x cm입니다.
//		System.out.print(name + "님은 " + age + "살이며, 사는 곳은 " 
//						  + address + "이고 키는 " + height 
//						  +"cm입니다.");
		 ...
		 * sc.nextXXX() 뒤에(다음 입력 받기 전에) sc.nextLine(); 구문 작성 => 버퍼 비워짐
		 */
	...
	public void inputScanner3() {
		...
		char gender = sc.nextLine().charAt(0); // => 'M'
		/*
		 * 특정문자열.charAt(뽑고자하는 인덱스번호) => 문자열로부터 해당 인덱스의 문자값
		 * ** 인덱스 : 0부터 시작
		 */
	...
	public void inputPriduct() {
		 ...
		 * 					 sc.nextLine(), sc.next(), sc.nextInt(),  
		 * * 주의사항
		 * sc.nextXXX() 메소드 뒤에 sc.nextLine()으로 문자열을 읽어
		 * 그 전에 sc.nextLine(); 더 써서 버퍼를 비워야함
		 * 
		 * '문자' 값을 입력받기 위해서 sc.nextLine으로 문자열로 읽어들인 후
		 * .charAt(0)
		 * 
		 * 
		 */
