...
public class A_Variable {//찐이름은 패키지까지 포함->com.br.variable.A_Variable이므로, 패키지 없는 A_Variable과는 이름이 달라서 공존 가능
	...
	public void printValue() {
		...
		int pay = 9000;	// 시급 * 변수에 의미를 주석달기, =은 대입연산자(뒤에껄 앞에)
	...
	public void declareVariable() {//변수 선언
		 ...
		 * - 특정영역(중괄호 블럭{})에 동일한 변수명으로 선언 불가
		 * 	 해당 영역에 선언된 변수는 그 영역에서만 사용 가능 (다른 메소드에서 사용불가) == 지역변수 개념
		 * * 변수 선언 후 값 대입
		 * [표현법] 변수명 = 값;
		 * 
		 * * 변수 선언과 동시에 값 대입
		...
		int iNum = 100000; // 4byte (정수형 중의 대표) -21억.. ~ 21억..
		long lNum = 10000000000L; // 8byte 
		
		// 2_2) 실수형
		float fNum = 4.123412341234f; // 4byte
		double dNum = 8.1234567890123456789; // 8byte (실수형 중의 대표) -> 더 큰 값x 더 정확한 값o(소수점 아래 더 길게 담을 수 o)
		...
		//번외(큰 값을 작성할 때 _를 이용해서 가독성 높일 수 있음)
		int etc = 999_999_999;
		...
		//여기부턴 오류는 안나지만 권장(개발자 매너)
		// 여러 단어일 경우 붙여쓰고 두번째 단어부터 첫글자 대문자 (낙타표기법)
		String username;
		String userName; // 관례상 맞는 표현
		
		String 이름; // 한글 가능하지만 권장 x(한글 사용 않는 환경에선 오류 발생시킬 수 있음)
				
	}
	public void constant() {//상수 = constant
		/*
		 * 5. 상수란
		 * 	  메모리 상에 값을 기록하는 공간인데
		 * 	  변수와의 차이점으로는 한 번 기록된 값을 변경하는게 불가능
		 * 
		 * 	  [표현법] final 자료형 상수명;
		 * 
		...
		System.out.println(Math.PI);// 자바에서 Math라는 클래스를 만들어둠. 그 안에 이미 파이란 상수 있음
		//Math.PI = 2.11111;
		//ctrl 누르고 마우스로 눌러보면 까볼 수 있음
		
	}
	
	/*
	 * **정리**
	 * - 값(data, 리터럴) : 프로그램 상에 필요한 명시적인 data / 또는 사용자가 키보드 또는 마우스로 입력한 data
	 * - 변수 : 값(리터럴)을 메모리 상에 기록하기 위한 박스같은 존재
