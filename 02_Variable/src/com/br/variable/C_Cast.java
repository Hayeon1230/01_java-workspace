...
public class C_Cast {
	 ...
	 * 
	 * * 컴퓨터에서의 값 처리 규칙
	 * 1. 대입연산자를 기준으로 왼쪽과 오른쪽의 자료형은 같아야 됨
	 * 	  --> 같은 자료형의 값만 대입이 가능하다.
	 * 	  --> 다른 자료형의 값을 대입하고자 한다면 형변환을 진행해야됨!!
	 * 
	 * 	  자료형 변수명 = (자료형)값;
	 * 
	 * 2. 같은 자료형끼리만 연산이 가능 => 계산 결과도 같은 자료형으로 나옴
	 * 	  값 + 값 => 두 값이 같은 자료형이여야됨
	 * 
	 * * 형변환의 종류
	...
	public void autoCasting() {
		...
		// float은 실수형이기 때문에 long형 보다 표현 가능한 수의 범위가 더 크기 때문
		long l5 = 1000;
		float f5 = /*(float)*/l5;
	...
	public void forceCasting() {
		// 강제형변환 : 큰 범위의 자료형을 작범위의 자료형으로 바꾸는 것
		...
		//int iSum = /*(double)*/iNum + dNum; //결과값이 double
		
		// 해결방법1. 연산결과를 int형으로 강제형변환 하면됨
		int iSum1 = (int)(iNum + dNum);
		System.out.println("iSum1 : " + iSum1); //10.0+5.89 => (int)15.89 => 15(소수점 아래 버림) => 데이터손실 발생할 수 있음
		
		// 해결방법2. double형 값만을 int형으로 강제형변환
		int iSum2 = iNum + (int)dNum; // 10 + 5 => 15
		System.out.println("iSum2 : " + iSum2);
		
		// 실수값을 정수형으로 강제형변환하는 순간 "데이터손실이 발생할 수 있음!!!" -> 신중
